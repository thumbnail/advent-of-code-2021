...
(defn horizontal? [[[x y] [x' y']]]
  (= y y'))

(defn vertical? [[[x y] [x' y']]]
  (= x x'))
...
(defn render [m]
  (run! (fn [xs]
          (apply println (map #(cond
                                 (zero? %) "."
                                 :else %) xs)))
        m))

(letfn [(apply-horizontal [m coordinates]
          (reduce (fn [m' [[x, y], [x', y']]]
        ...
        (apply-vertical [m coordinates]
          (reduce (fn [m' [[x, y], [x', y']]]
                  ...
                  m
                  coordinates))
        (apply-diagonal [m coordinates]
          (reduce (fn [m' [[x, y], [x', y']]]
                    (let [coords ((fn [[[x y :as coord] :as coords]]
                                    (if (= coord [x' y'])
                                      coords
...
(comment
 (true? (horizontal? (first example-input))))
